...
//            //"add_edit_note?title={title}&noteId={noteId}"
//            arguments = listOf(
//                navArgument(NoteDestinations.TITLE_ARG) { //title
//                    type = NavType.IntType
//                    defaultValue = R.string.add_note
//                },
//                navArgument(NoteDestinations.NOTE_ID_ARG) {//note_id
//                    type = NavType.StringType
//                    nullable = true
//                    defaultValue = null
//                }
...
//used to go in addEdit onNavigateToAllSales
/*navController.previousBackStackEntry
                        ?.savedStateHandle
                        ?.set(SALE_RESULT_KEY, resultCode) //this value can be retrieved from "currentBackStackEntry" in the screen we pop back to
...
/*
 ...
 // observe result in nav host scope
            val savedState = navController.currentBackStackEntry?.savedStateHandle

/**
             * This below is step #5. Here, get the backStackEntry, get the savedStateHandle from it,
             * then below we get the state flow<String> it has in the same key as we stored it. Here we
             * define what message to send, and for each result attained from the savedState.getStateFlow
             * we call [AllSalesViewModel.emitSnackbar] and pass the message we want to display.
             * @see AllSalesViewModel.emitSnackbar for the next step
             */
            LaunchedEffect(savedState) {
                savedState?.getStateFlow<String?>(SALE_RESULT_KEY, null)?.collect { result ->
                    Log.d(TAG, "savedState getStateFlow result is [$result]")
                    if (result != null) {
                        ...
                        if (msgRes != null) {
                            // call vm to emit snackbar event
                            msgRes.let { allSalesViewModel.emitSnackbar(msgRes) }
                            Log.d(TAG, "MessageRes is not null. Calling emitSnackbar")
                        }
                        // clear
                        savedState[SALE_RESULT_KEY] = null
                    } else Log.d(TAG, "messageRes is null")
                }
            }
